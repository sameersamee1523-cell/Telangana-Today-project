...
import { BookOpen, Clock, CheckCircle, AlertTriangle, Eye, Filter, ArrowRight, Circle, Users, Pencil, Star, Send, Activity, Plus } from 'lucide-react'
...
export default function EditorDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [storiesRes, statsRes, catRes, allStoriesRes] = await Promise.all([
          storiesAPI.getAll({ status: 'submitted,under_review', limit: 8 }),
          analyticsAPI.getDashboardStats(),
          analyticsAPI.getCategoryWise(),
          storiesAPI.getAll({ limit: 5, sort: 'created_at', order: 'desc' }),
        ])
        const reviewQueue = storiesRes?.stories || []
        const allStories  = allStoriesRes?.stories || []
        setStories(reviewQueue)
        setStats(statsRes)
        setRecentStories(allStories)
  ...
  if (loading) return <AppLayout title="Editor Dashboard"><div className="h-full flex items-center justify-center"><LoadingSpinner size="lg" text="Loading..." /></div></AppLayout>

  const overdueCount = stats?.overdueStories?.length ?? 0
    ...
    <AppLayout title="Editor Dashboard">
      <div className="space-y-6 pb-6">
          ...
          <div className="relative flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-bold uppercase tracking-widest" style={{ color: 'rgba(148,163,184,0.8)' }}>Editorial Desk</span>
              </div>
              <h2 className="text-2xl font-black text-white mb-1" style={{ letterSpacing: '-0.02em' }}>Editor Dashboard</h2>
              <p className="text-sm" style={{ color: 'rgba(148,163,184,0.75)' }}>
                {stories.length} stories awaiting review · {overdueCount > 0 ? `${overdueCount} overdue` : 'No overdue'}
              </p>
            </div>
            <Link to="/stories" className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all duration-200"
              style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.8)' }}>
              <Filter size={13} /> Review Queue
            </Link>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatsCard title="Under Review"    value={stats?.pendingStories ?? 0}   icon={Eye}          color="#3B82F6" delay={0.05} />
          <StatsCard title="Published"       value={stats?.publishedStories ?? 0} icon={CheckCircle}  color="#10B981" delay={0.1}  />
          <StatsCard title="Overdue"         value={overdueCount}                 icon={AlertTriangle}color="#F59E0B" delay={0.15} />
        </div>
        ...
        {/* Bottom Row: Review Queue + Activity Feed */}
        <div className="grid lg:grid-cols-2 gap-6">

          {/* Review Queue (Recent Stories for editor) */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card p-6">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h3 className="font-bold text-surface-900 dark:text-white text-base">Stories Awaiting Review</h3>
                <p className="text-xs text-surface-400 mt-0.5">Submitted by reporters for your review</p>
              </div>
              <Link to="/stories"
            ...
            {stories.length > 0 ? (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-surface-400 mt-0.5">
                          By {s.reporter_name} · {s.category_name} · Due {formatDate(s.deadline)}
                        </p>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0">
                        <PriorityBadge priority={s.priority} />
                        {isOverdue(s.deadline) && <AlertTriangle size={13} className="text-primary-500" />}
                      </div>
                    </Link>
                  </motion.div>
                ))}
              ...
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-sm font-medium text-surface-500">Review queue is clear!</p>
                <p className="text-xs text-surface-400 mt-1">Great work keeping up with submissions</p>
              </div>
            )}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="card p-6">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-surface-400 mt-0.5">Recent newsroom events</p>
              </div>
            ...
            <div className="space-y-1">
              {[
                { icon: Users,  color: '#3B82F6', bg: '#EFF6FF',  label: 'Story Assigned',   title: recentStories[0]?.title, sub: recentStories[0]?.reporter_name, time: recentStories[0]?.created_at },
                { icon: Send,   color: '#F59E0B', bg: '#FFFBEB',  label: 'Story Submitted',  title: stories[0]?.title,        sub: stories[0]?.reporter_name,        time: stories[0]?.created_at },
                { icon: Eye,    color: '#8B5CF6', bg: '#F5F3FF',  label: 'Review Completed', title: recentStories[1]?.title, sub: recentStories[1]?.reporter_name,  time: recentStories[1]?.created_at },
                { icon: Star,   color: '#10B981', bg: '#ECFDF5',  label: 'Story Published',  title: recentStories[2]?.title, sub: recentStories[2]?.reporter_name,  time: recentStories[2]?.created_at },
                { icon: Plus,   color: '#E11D48', bg: '#FFF1F3',  label: 'Reporter Active',  title: 'Team members online',   sub: `${stats?.activeReporters || 0} reporters active`, time: null },
              ].filter(a => a.title).map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-surface-400">{item.sub}{item.time ? ` · ${formatRelative(item.time)}` : ''}</p>
                    </div>
                  </motion.div>
                )
              })}
            ...
            {overdueCount > 0 && (
              <div className="mt-4 pt-4" style={{ borderTop: '1px solid #F1F5F9' }}>
                <div className="p-3 rounded-xl flex items-center gap-3"
                  ...
                  <AlertTriangle size={14} className="text-primary-500 shrink-0" />
                  <p className="text-xs font-semibold text-primary-600 flex-1">
                    {overdueCount} overdue {overdueCount === 1 ? 'story' : 'stories'}
                  </p>
                  <Link to="/stories" className="text-xs font-bold text-primary-500">Review →</Link>
                </div>
              </div>
            )}
          </motion.div>
