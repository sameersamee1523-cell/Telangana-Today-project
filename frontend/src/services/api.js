...
export const analyticsAPI = {
  getDashboardStats: async () => {
    ...
    // Backend returns: { success: true, data: { storyStats, active_reporters, recentActivity, overdueStories } }
    // Axios interceptor unwraps the HTTP body, so res = { success, data: {...} }
    // We need res.data to reach the actual payload
    const d = res?.data || {}
    return {
      totalStories:    Number(d.storyStats?.total_stories)  || 0,
      activeReporters: Number(d.active_reporters)           || 0,
      pendingStories:  Number(d.storyStats?.pending_review) || 0,
      publishedStories:Number(d.storyStats?.published)      || 0,
